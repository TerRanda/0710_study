...
//        인증 유저 정보의 principal 을 가져온다.
        OAuth2User oAuth2User = (OAuth2User) authentication.getPrincipal();
//        가져온 principal의 email(username)을 통해서 유저에 대한 모든 정보를 db에서 가져온다.
        User user = userService.findByEmail((String) oAuth2User.getAttributes().get("email"));
//        가져온 유저 정보로 refresh 토큰 생성
        String refreshToken = tokenProvider.generatedToken(user, REFRESH_TOKEN_DURATION);
//        생성된 refresh토큰을 refresh 테이블에 저장.
        saveRefreshToken(user.getId(), refreshToken);
//        생성된 토큰을 브라우저 쿠키에 저장
        addRefreshTokenToCookie(request, response, refreshToken);
//        가져온 유저정보를 통해서 access토큰을 생성
        String accessToken = tokenProvider.generatedToken(user, ACCESS_TOKEN_DURATION);
//        access 토큰값과 이동할 targetURL 설정
